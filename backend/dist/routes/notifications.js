...
const deleteTokenBodySchema = zod_1.z.object({
    token: zod_1.z.string().min(1, 'Token is required'),
});
...
router.delete('/token', validateBody(deleteTokenBodySchema), notification_1.removeToken);
router.patch('/preferences', auth_1.requireAuth, validateBody(updatePreferencesBodySchema), notification_1.updatePreferences);
router.delete('/token/:tokenId', auth_1.requireAuth, notification_1.removeTokenById);
router.post('/simulate', auth_1.requireAuth, validateBody(simulatePushBodySchema), notification_1.simulatePush);
